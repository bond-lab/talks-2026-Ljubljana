...
\title[wnja + OMW]{The Japanese Wordnet and the Open Multilingual Wordnet}
...
\item 2006-2009 NICT (National Inst. for Info. and Comm. Technology)
  ...
  \item Open Multilingual Wordnet
  \end{itemize}
\framebreak
\item 2009-2022 NTU (Nanyang Technological University)
  ...
  \end{itemize}
\item 2022- UPOL (Palacký University, Olomouc)
  \begin{itemize}
...
\section{The Japanese Wordnet}


\begin{frame}{What is a Wordnet?}
...
\begin{frame}{NICT decided to build a wordnet for Japanese}
\begin{itemize}
\item Stage 0
  \begin{itemize}
  \item Semi-automatically translate English WordNet (3.0)
  \end{itemize}
\item Stage 1 
  \begin{itemize}
  \item Manually correct the top 10,000 entries
  \item This includes the 5,000 \textbf{core} synsets
  \end{itemize}
\item Stage 2 (in progress: poster yesterday)
  \begin{itemize}
  \item Correct the next most frequent 15,000 entries
  \item Create a Japanese version of SemCor
\item Release WordNet-ja v1.0
\end{itemize}
\end{itemize}

\end{frame}

\begin{frame}{Exploit other wordnets}

\begin{itemize}
\item For each synset in WordNet 3.0
  \begin{itemize}
  \item Find its equivalents in WN-Fr, WN-Es, Wn-De
  \item Look up translations for all equivalents
    \{J$_e$\}, \{J$_f$\}, \{J$_s$\}, \{J$_d$\}
  \item Rank Japanese equivalents
    \\ score $s = |$links$| + 10$ for links in two languages   
  \end{itemize}
\end{itemize}

The result is a WordNet with  multiple Japanese candidates for each
synset ranked by score.
\end{frame}

\begin{frame}{Linking with Multiple WordNets}
  \begin{tikzpicture}[scale=1.4]
    \tikzstyle{word}=[rectangle,rounded corners,draw]
    % \tikzstyle{every node}=[rectangle,rounded corners,draw]
    \fill[color=gray!15] (-1,-1.8) rectangle +(8,2.5); 
    \node[color=black] at (0,.5) {\wn{bat#n#1}};
    \node at (-1.5,.5) {\includegraphics[width=3em]{img/bat.eps}};

    \node at ( 0.1,-1.4) (chiropteran) [word] {\eng{chiropteran}}; 
    \node at ( 0,-.3) (bat1) [word] {\eng{bat}}; 
    \node at ( 3.0,0.2)  (chso)[word] {\fra{chauve-souris}}; 
    \draw [->] (bat1) -- node[auto]  {\small Wn-Fr} (chso);
    \draw [->] (bat1) -- node[left]  {\small Wn-En} (chiropteran);

    \node at ( 6,-1) (koumori) [word] {\ja{蝙蝠}}; 

   \fill[color=gray!15] (-1,-4.5) rectangle +(8,2.4); 
    \node[color=black] at (0,-2.25) {\wn{bat#n#5}};
    \node at (-1.5,-3) {\includegraphics[width=2em]{img/baseball-bat.eps}};
    \node at ( 6,-3) (batto)  [word] {\ja{バット}};

    \node at ( 0,-3) (bat5) [word] {\eng{bat}}; 
    \node at (3.0,-4) (batte)[word] {\fra{batte}}; 
    \node at (0.3,-4) (gourdin)[word] {\fra{gourdin}}; 
    \draw [->] (bat5) --  node[auto] {\small Wn-Fr} (batte);
    \draw [->] (bat5) --  node[left] {\small Wn-Fr} (gourdin);

    \draw [thick,->] (bat1) -- node[auto] {\small J-E} (koumori); 
    \draw [->] (bat1) -- node[auto] {\small J-E}(batto); 
    \draw [->] (bat5) -- node[auto] {\small J-E}(koumori); 
    \draw [thick,->] (bat5)  -- node[auto] {\small J-E}(batto);
    \draw [thick,->] (chso) -- node[auto] {\small J-F} (koumori); 
    \draw [thick,->] (batte) --  node[auto] {\small J-F} (batto);
  \end{tikzpicture}
...
\begin{frame}{Japanese Wordnet 2.0}
   ...
   \begin{itemize}
   \item We kept expanding the wordnet and adding more information, but it still did not describe the conceptual structure of Japanese.  
   \item So we started to make it describe Japanese better

     \begin{itemize}
     \item Taking advantage of a new format (GWA XML 1.0)
       \begin{itemize}
       \item Allows for variant forms
       \item Structure can be different from PWN (788 new concepts (synsets))
...
\begin{frame}{Orthographic Variants}
  ...
  \item synsetID=02765464-v (Eng: absorb, take in)
    ...
    \\ \textbf{吸収} (キュウシュウ, きゅうしゅう) +vs
    \\ \textbf{吸引} キュウイン, きゅういん) +vs
