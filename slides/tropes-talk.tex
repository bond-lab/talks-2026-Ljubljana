...
\author[Bond and many more]{Francis \textbf{Bond} 
  \\ Rowan Hall \textbf{Maudslay} and many, many more}
\institute[Palacký]{Asian Studies  \& Sinofon Center
  \\ General Linguistics
\\[1ex]  Palacký University, Olomouc, Czechia
\\[1.5ex] \url{<bond@ieee.org>}}

\date[JOTA 2026]{University of Ljubljana, April 2026}
...
\definecolor{metaBlue}{RGB}{72,120,207}
\definecolor{metoRed}{RGB}{180,84,80}
...
\begin{frame}{We are trying a different approach}
  ...
  \item ChainNet links word senses by metaphor --- for most nouns in the English wordnet
  \item Use ChainNet to study metaphor over a (more) complete vocabulary
    \begin{itemize}
...
\item  For every word with more than two senses
  \begin{itemize}
  \item examine all pairs (in wordnet)
  \item determine if they are  linked by metaphor or metonymy, or not at all
  \end{itemize}
  \begin{tcolorbox}
    \wns{darkness}{1} --- absence of light or illumination \\
...
\begin{frame}{Exploit wordnet for a richer view}
  ...
  \item Once we have senses, we can look at supersenses
    \begin{tcolorbox}
      \wns{darkness}{1} (\topic{state}) $\rightarrow$  \wns{darkness}{3} (\topic{state})  \\
      \wns{darkness}{1} (\topic{state}) $\rightarrow$  \wns{darkness}{4} (\topic{cognition}) 
    \end{tcolorbox}
   \item Or indeed any hypernym
    \begin{tcolorbox}
      \wns{darkness}{1} (illumination|state|attribute|TOP)  $\rightarrow$ \\
      \hspace*{1em} \wns{darkness}{3} (wickedness|condition|state|attribute|TOP)  \\
      \wns{darkness}{1} (illumination|state|attribute|TOP)) $\rightarrow$  \\
       \hspace*{1em} \wns{darkness}{4} (unenlightenment|ignorance|knowledge|\ldots{}|T.) 
    \end{tcolorbox}
...
\begin{frame}{We want to group metaphors}

  \begin{itemize}
  \item UNDERSTANDING IS LIGHT
    \begin{itemize}
    \item \eng{That talk left us in the \ul{dark}}
    \item \eng{I have seen the \ul{light}}
    \item \eng{That was \ul{illuminating}}
    \end{itemize}
  \item GOODNESS IS LIGHT
    \begin{itemize}
    \item \eng{Come away from the \ul{dark} side}
    \item \eng{Those are \ul{dark} deeds}
    \item \eng{They were the \ul{light} of my life}
    \end{itemize}
  \end{itemize}
...
%  \item We measure the number of synonyms of source and target
  \item We measure the inclusiveness (low is inclusive, high is specific) $\approx$ abstractness
  \item We measure the sentiment \citep{Bond:Choo:2022}
  \item We also look at the distribution of the domains \citep{10.1007/978-3-642-28604-9_2}
  \end{itemize}
  \begin{center}
...
\begin{frame}{Path Distance: Metaphor bridges distant concepts}
\textbf{Path distance} = $\dfrac{1}{1 + \text{steps in WordNet graph}}$

\medskip
\textbf{Metaphor}: \eng{\ul{sloth}} (animal $\to$ laziness)

\medskip
\begin{center}
\begin{tikzpicture}[
  every node/.style={font=\scriptsize, draw=gray!50, rounded corners=2pt,
                     fill=white, inner sep=2.5pt, align=center},
  edge from parent/.style={draw=gray!50, thin},
  level distance=8.5mm, sibling distance=30mm]
\node[fill=gray!15] {\textit{entity}}
  child { node {\textit{abstraction}}
    child { node {\textit{attribute} $\cdots$}
      child { node[fill=metaBlue!25, draw=metaBlue] {\textit{sloth}\\{\tiny laziness}} }
    }
  }
  child { node {\textit{organism}}
    child { node {\textit{animal} $\cdots$}
      child { node[fill=metoRed!25, draw=metoRed] {\textit{sloth}\\{\tiny mammal}} }
    }
  };
\end{tikzpicture}
\end{center}
...
\medskip
{\scriptsize $\approx$20 steps through \textit{entity} $\Rightarrow$ path dist.\ $= 0.05$}
\end{frame}

\begin{frame}{Path Distance: Metonymy stays close}
\textbf{Metonym}: \eng{\ul{radish}} (root $\to$ plant)
...
\begin{tikzpicture}[
  every node/.style={font=\scriptsize, draw=gray!50, rounded corners=2pt,
                     fill=white, inner sep=2.5pt, align=center},
  edge from parent/.style={draw=gray!50, thin},
  level distance=8.5mm, sibling distance=26mm]
\node {\textit{plant}}
  child { node[fill=metoRed!25, draw=metoRed] {\textit{radish}\\{\tiny the plant}} }
  child { node {\textit{herb}}
    child { node[fill=metaBlue!25, draw=metaBlue] {\textit{radish}\\{\tiny the root}} }
  };
\end{tikzpicture}
\end{center}

\smallskip
{\scriptsize 2 steps $\Rightarrow$ path dist.\ $= 0.33$}
...
\midrule
same synset & 1.00 \\
direct parent/child & 0.50 \\
\textit{radish} root $\leftrightarrow$ plant & 0.33 \\
\textit{sloth} mammal $\leftrightarrow$ laziness & 0.05 \\
\bottomrule
\end{tabular}

\medskip
\textbf{Prediction}: metaphors bridge more distant concepts
\end{frame}
...
\begin{frame}{Depth: Shallow = Abstract}
\begin{center}
\begin{tikzpicture}[
  every node/.style={font=\footnotesize, draw=gray!50, rounded corners=2pt,
                     fill=white, inner sep=2pt, align=center, minimum width=2.1cm},
  grow=down, level distance=9.5mm, sibling distance=6mm]
\node[fill=gray!20] {\textit{entity} \quad \textbf{0}}
  child { node[fill=gray!15] {\textit{abstraction} \quad \textbf{1}}
    child { node {\textit{psychological} \textbf{2}}
      child { node {\textit{emotion} \quad \textbf{3}}
        child { node[fill=blue!15] {\textit{love} \quad \textbf{4}} }
      }
    }
  };
\end{tikzpicture}
...
\medskip
\begin{itemize}
  \item \textbf{Shallow} (low depth) $\approx$ general/abstract concept
  \item We use \texttt{synset.min\_depth()} — shortest path to root
\end{itemize}
\end{frame}

\begin{frame}{Depth: Deep = Concrete}
\begin{center}
\begin{tikzpicture}[
  every node/.style={font=\footnotesize, draw=gray!50, rounded corners=2pt,
                     fill=white, inner sep=2pt, align=center, minimum width=2.2cm},
  grow=down, level distance=9.5mm]
\node[fill=gray!20] {\textit{entity} \quad \textbf{0}}
  child { node[fill=gray!15] {\textit{physical entity} \textbf{1}}
    child { node {\textit{object} \quad \textbf{2}}
      child { node {\textit{food} \quad \textbf{3}}
        child { node {\textit{fruit} \quad \textbf{4}}
          child { node[fill=orange!20] {\textit{apple} \quad \textbf{5}} }
        }
      }
    }
  };
...
\medskip
\begin{itemize}
  \item \textbf{Deep} (high depth) $\approx$ specific/concrete concept
  \item \textbf{Prediction}: metaphor source is more concrete (deeper) than target
\end{itemize}
\end{frame}
...
% \begin{frame}{Synonymy Density: How Many Words Share This Meaning?}
% WordNet groups synonyms into \textbf{synsets}.
% Synonymy density = number of lemmas in the synset.
...
% \midrule
% \textit{car} & 5 \\
%   \ \footnotesize (car, auto, automobile, \ldots & \\[2pt]
%   \ \ \ \footnotesize machine, motorcar) & \\[2pt]
% \textit{emotion} & 1 \\
% \textit{fruit} & 1 \\
% \textit{chestnut (fruit)} & 1 \\
% \textit{chestnut} (tree) & 2 \\
% \textit{love} (feeling) & 3 \\
% \ \footnotesize (love, passion, adoration) & \\
% \bottomrule
% \end{tabular}
% \end{frame}

% \begin{frame}{Synonymy Density: Why Measure It?}
% Many synonyms $\Rightarrow$ concept is
% \emph{lexically entrenched} --- many
% words compete to express it, suggesting
% high communicative pressure.

% \medskip
% Concepts used as \textbf{source} or
% \textbf{target} of tropes may differ:
% \begin{itemize}
%   \item Do sources/targets tend to be
%         more or less lexically rich?
%   \item Does this differ between
...
\begin{frame}{Inclusiveness: How Much of the Conceptual Space Does It Cover?}
If concept $c$ has $n$ descendants (direct + indirect)
among $N$ total noun synsets:
\[
  \mathrm{inclusiveness}(c) = -\log\!\left(\frac{n+1}{N}\right)
\]
...
Concept & Desc.\ $n$ & Incl. \\
\midrule
\textit{entity} (root) & 82{,}114 & $\approx$ 0.0 \\
\textit{animal}        & $\approx$8{,}000 & $\approx$ 2.3 \\
\textit{dog}           & $\approx$60  & $\approx$ 7.0 \\
\textit{poodle} (leaf) & 0        & $\approx$ 11.3 \\
\bottomrule
\end{tabular}
\end{center}
\medskip
{\small $N = 82{,}115$ noun synsets in \texttt{omw-en:1.4}}
\end{frame}

\begin{frame}{Inclusiveness: General vs Specific}
\textbf{Low} value ($\approx 0$) = very \emph{general} concept \\
\textbf{High} value ($\approx 11$) = very \emph{specific} concept

\medskip
Finer-grained than simple depth:
two concepts at the same depth can
have very different numbers of
descendants.

\medskip
\textbf{Prediction}: tropes map
\emph{from} inclusive (general) sources
\emph{to} specific targets
\end{frame}

\begin{frame}{Inclusiveness (Low is general --- High is specific)}
  \includegraphics[height=0.9\textheight]{img/inclusiveness_comparison_errorbar.png}
\end{frame}


%%%% --- Sentiment ---

\begin{frame}{Sentiment: Is the Concept Positive or Negative?}
From \citet{Bond:Choo:2022}: a sense-level
lexicon built by combining hand-crafted
lexicons and a sense-tagged corpus, then
propagating scores across the WordNet graph.
...
Sense & Score \\
\midrule
\textit{love} (feeling) & $+0.52$ \\
\textit{victory} (success) & $+0.38$ \\
\textit{table} (furniture) & $\phantom{+}0.00$ \\
\textit{prison} (building) & $-0.31$ \\
\textit{murder} (killing) & $-0.75$ \\
\bottomrule
\end{tabular}
\end{center}

\medskip
\textbf{Baseline}: mean score of all single-word nouns $= -0.045$\\
{\small (slight negativity bias in the lexicon)}
\end{frame}

\begin{frame}{Sentiment: Scale and Prediction}
\begin{center}
\begin{tikzpicture}
% x-coords = sentiment score × 2.5 cm
\draw[thick, -{stealth}] (-4.5,0) -- (4.75,0);
\foreach \x/\l in {-3.75/{$-1$}, 0/{$0$}, 3.75/{$+1$}} {
  \draw (\x, 0.06) -- (\x, -0.06);
  \node[font=\small] at (\x, -0.32) {\l};
}
\node[font=\small, align=center] at (-3.75, 0.50) {very\\neg.};
\node[font=\small]               at (0,     0.50) {neutral};
\node[font=\small, align=center] at ( 3.75, 0.50) {very\\pos.};
% dots: murder −1.125×2.5=−2.81, table 0→0, victory 1.17×2.5=2.93
\fill[metoRed!80]  (-2.8125, 0) circle (3pt);
\node[below=15pt, font=\footnotesize] at (-2.8125, 0) {\textit{murder}};
\fill[gray!60]     (0,       0) circle (3pt);
\node[below=15pt, font=\footnotesize] at (0,       0) {\textit{table}};
\fill[metaBlue!80] ( 2.925,  0) circle (3pt);
\node[below=15pt, font=\footnotesize] at ( 2.925,  0) {\textit{victory}};
% noun average: −0.067×2.5=−0.168
\draw[dashed, gray] (-0.168, -0.65) -- (-0.168, 0.70)
  node[above, font=\footnotesize, gray] {noun avg.};
\end{tikzpicture}
\end{center}

\medskip
Coverage: 68\% of source senses, 52\% of target senses annotated.

\medskip
\textbf{Prediction}: tropes may prefer more neutral/positive source concepts
\end{frame}

\begin{frame}{Sentiment (-1.0 to +1.0)}
  \includegraphics[height=0.9\textheight]{img/sentiment_comparison_errorbar.png}
\end{frame}

%%%% --- Domains ---

\begin{frame}{Domains: Two Classification Systems}
\textbf{WordNet topics} (lexicographer files)\\[2ex]
{\small 45 broad editorial labels assigned during WordNet construction
(e.g.\ \topic{noun.animal}, \topic{noun.artifact}, \topic{noun.cognition}).\\
Designed to organise lexicographic work ---
\emph{not} cognitive conceptual domains.}

\medskip
\textbf{eXtended WordNet Domains} \citep{10.1007/978-3-642-28604-9_2}\\[2ex]
{\small 170 subject-domain labels from the Dewey Decimal Classification
(e.g.\ \dom{gastronomy}, \dom{medicine}, \dom{sport}, \dom{animals}).\\
Propagated to all 82k noun synsets by a graph-based algorithm
(UKB) using WordNet relations + gloss links.}

\medskip
Each synset is assigned its \textbf{highest-scoring} domain.
\end{frame}

\begin{frame}{Domains: Do Metaphors Cross Domains?}
\textbf{Example:}

\medskip
\eng{I ate a \ul{chestnut}} \\
\hspace{2em} $\to$ \textit{chestnut} (nut) \\
\hspace{2em} \topic{noun.plant} \quad \dom{gastronomy}

\medskip
\eng{I pruned the \ul{chestnut} tree} \\
\hspace{2em} $\to$ \textit{chestnut} (tree) \\
\hspace{2em} \topic{noun.plant} \quad \dom{plants}

\bigskip
\textbf{Key question:} \\
Do metaphors cross domains more than metonyms? \\[4pt]
\textbf{Theory} (Kövecses): \\
Metonymy = within-domain \\
Metaphor = across domains
\end{frame}
...
\begin{frame}{Do Metaphors Cross Domains?}


\medskip
\begin{itemize}
  \item \textbf{Same-category rate}: of all tropes, what fraction have source and target in the \emph{same} category?
\\        \quad{\small(higher $=$ more within-domain)}
  % \item \textbf{NMI} (Normalized Mutual Information): if you know the source category, how well can you predict the target?
  % \\      \quad{\small(0 $=$ no help at all;\; 1 $=$ perfectly predictable)}
\end{itemize}

% \begin{tabular}{lcc cc}
% \toprule
% & \multicolumn{2}{c}{\textit{WN Topics} (26)}
% & \multicolumn{2}{c}{\textit{XWND Domains} (170)} \\
% \cmidrule(lr){2-3}\cmidrule(lr){4-5}
% & {\color{metaBlue}Metaphor} & {\color{metoRed}Metonymy}
% & {\color{metaBlue}Metaphor} & {\color{metoRed}Metonymy} \\
% \midrule
% Same-cat.\ rate
%   & 0.42 & 0.28   & 0.11 & 0.21 \\
...
Metonymy stays within a domain nearly \textbf{twice} as often as metaphor (21\% vs.\ 11\%),
and the source domain is a much stronger predictor of the target (NMI 0.41 vs.\ 0.32).

% \medskip

% \textbf{Why do WN topics mislead?}\\
% Lexicographer files group words with similar meanings, but not
% necessarily about different things.  They are also quite broard (26 topics for nouns).  XWND domains reflect broader conceptual
% structure, not lexical organisation, with more fine-grained description (170 domains).
...
\begin{frame}{Structural Patterns: What the Numbers Show}
  \begin{itemize}
  \item Metaphors are more distant in the network than metonyms
  \item Source is more general than target (for both metaphors and metonyms, more so for metaphors) 
  \item There are many significant differences but almost all with negligible effect size
    \begin{itemize}
    \item We need to categorize metaphors/metonyms further to see interesting differences.
    \end{itemize}
  \item Metonyms are more likely to share the same domain between
    source and target than metaphors, as \citet{Kovecses2010Metaphor}
    predicted.
  \end{itemize}
...
\begin{frame}{Can we see how English metaphors are used cross-lingually?}
  ...
  \item For example, are translations of linked pairs likely to be
    ...
    \begin{itemize}
    \item If so then metaphor/metonymy holds cross language
...
\begin{frame}{Wordnets and the Open Multilingual Wordnet}
  \begin{itemize}
  \item A \txx{wordnet} is a lexical database organising words into
    \txx{synsets} (synonym sets), each representing a unique concept,
    linked by semantic relations: hypernymy, hyponymy, meronymy, \ldots
  \item Over 50 hand-built wordnets exist; automatically-built wordnets
    cover over 1{,}200 languages (Wiktionary, Swadesh lists, \ldots)
  \item The \txx{Open Multilingual Wordnet} (OMW) links them all to
    Princeton WordNet senses under open licenses \citep{Bond:Foster:2013}
  \item \textbf{Key point for this talk}: if two English senses share a
    translation in another language, that language encodes the same
    semantic connection --- so OMW translations let us test how far
    English metaphors and metonymies travel cross-linguistically
  \end{itemize}
\end{frame}
...
\begin{frame}{How are wordnets built?}
  ...
  \begin{itemize}
  \item Most new wordnets built by the \txx{expand} method --- the
    structure of English is used as a base, and new lemmas are added to the synsets
  \item Some wordnets are built by the \txx{merge} method --- an existing lexical resource is merged with the English Wordnet
...
\begin{frame}[allowframebreaks]{We do this for all the lexicons in OMW-1.4}
  ...
  \begin{center}
  \begin{tabular}{lrrrr}
    Lang & Unlinked & Metaphor & Metonymy & Non-Zero Instances \\ \hline
...
Mean & 0.78 & 1.39 & 1.96 &  3,774\makebox[0mm]{~~~.3} 
  ...
  Unlinked  share fewer translations, metaphor shares more and metonymy shares much more.

  \Large Metonymy is more likely to hold cross linguistically!
...
\item We can make a searchable database where the edges are decorated by the morphological differences
  ...
  \item Another way to group the tropes together in families
...
\item With ChainNet we can go bottom-up and find many more patterns
  ...
  \end{itemize}
\item We are linking this with our mapping to Conceptual Metaphors
\end{itemize}
...
\begin{frame}{We will mark metaphor/metonymy in more languages}
   \begin{tcolorbox}
...
\item The combination of detailed annotation of meaning (ChainNet \& Wordnet) and
  ...
  \item Metaphor pairs are further apart than Metonym pairs
  \end{itemize}
...
\begin{frame}{Toward a broader understanding of Metaphor and Metonymy}
  \begin{itemize}
...
\begin{frame}{Thanks and Resources}
  ...
  \begin{itemize}
  \item Thanks to Erasmus for supporting this visit
  ...
  \item Parts of this talk were presented at the 2026 LOT winter
    school Schultink lecture, 
    RegICON 2025 and Sprogteknologisk Konference 2024
