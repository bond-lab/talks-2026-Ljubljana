...
\begin{frame}{Differences Between Metaphors and Metonyms}
  ...
  \end{itemize}
  \begin{center}
    These are new results (they will be presented at the RAAM workshop in June)
  \end{center}
\end{frame}

%%%% --- Path Distance ---

\begin{frame}{Path Distance: How far apart are source and target?}
\begin{columns}[T]
...
\smallskip
{\small \textbf{Prediction}: metaphors bridge
...
\begin{frame}{Depth: Where in the Hierarchy Does the Concept Sit?}
\begin{columns}[T]
...
% \medskip
% Similar to \emph{information content} in NLP
% --- but based purely on WordNet structure,
...
\begin{frame}{Information Content: Corpus-Weighted Specificity}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
IC is based on \emph{corpus frequency}, not WordNet structure:
\[
  \mathrm{IC}(c) = -\log p(c), \quad
  p(c) = \frac{\mathrm{freq}(c)}{\mathrm{freq}(\mathrm{root})}
\]
where $\mathrm{freq}(c)$ accumulates counts for $c$ and all
its descendants in a sense-tagged corpus (SemCor).

\medskip
\begin{tabular}{lcc}
\toprule
Concept & SemCor freq & IC \\
\midrule
\textit{entity} (root) & 128{,}767 & $0.0$ \\
\textit{animal}        & $\approx$5{,}000 & $\approx$ 3.2 \\
\textit{dog}           & $\approx$200 & $\approx$ 6.4 \\
\textit{poodle}        & $\approx$2  & $\approx$ 11.0 \\
\bottomrule
\end{tabular}
\end{column}
\begin{column}{0.45\textwidth}
\textbf{Inclusiveness} (structural): counts
descendants in the WordNet graph.

\medskip
\textbf{IC} (empirical): counts how often
a concept appears in real text.

\medskip
Both measure specificity, but IC reflects
\emph{how language is actually used} ---
a rare concept scores high even if
it has many siblings in WordNet.

\medskip
{\small Coverage: $\approx$67\% of source
senses, $\approx$50\% of target senses
attested in SemCor.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Information Content (Low is general --- High is specific)}
  \includegraphics[height=0.9\textheight]{img/ic_comparison_errorbar.png}
\end{frame}
...
\medskip
\textbf{Baseline}: mean score of all\\
single-word nouns $= -0.045$\\
...
\foreach \x/\l in {-1.5/{$-1$}, 0/{$0$}, 1.5/{$+1$}} {
  \draw (\x, 0.06) -- (\x, -0.06);
  \node[font=\footnotesize] at (\x, -0.25) {\l};
}
\node[font=\footnotesize, align=center] at (-1.5, 0.35) {very\\neg.};
\node[font=\footnotesize] at (0, 0.35) {neutral};
\node[font=\footnotesize, align=center] at (1.5, 0.35) {very\\pos.};
\fill[metoRed!80]  (-1.125, 0) circle (3.5pt)
  node[below=4pt, font=\scriptsize] {\textit{murder}};
\fill[gray!60]   (0, 0) circle (3.5pt)
  node[below=4pt, font=\scriptsize] {\textit{table}};
\fill[metaBlue!80]  (1.17, 0) circle (3.5pt)
  node[below=4pt, font=\scriptsize] {\textit{victory}};
\draw[dashed, gray] (-0.067, -0.5) -- (-0.067, 0.6)
  node[above, font=\scriptsize, gray] {noun avg.};
\end{tikzpicture}

\bigskip
Coverage: 68\% of source senses,\\
52\% of target senses annotated.

\medskip
\textbf{Prediction}: tropes may prefer
more neutral/positive source concepts
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Domains: Categorising Concepts Beyond WordNet}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
\textbf{WordNet topics} (lexicographer files)\\
...
\medskip
\textbf{eXtended WordNet Domains} \citep{10.1007/978-3-642-28604-9_2}\\
...
\bigskip
\textbf{Key question:} \\
Do metaphors cross domains more
than metonyms? \\[4pt]
...
\begin{itemize}
    \item Developing wordnets requires careful analysis \\ $\Rightarrow$ impossible to automate
    \item Word meaning can be culturally specific, \\ $\Rightarrow$ requires careful consideration of local concepts.
